...
def ask_if_overwrite_existing_file(anyKeyfilePathNoQuotes):
        existingFileOverwrite = str(input("\n\nThis file exists. Overwrite? (y/n): "))
        if existingFileOverwrite == ("y" or "Y"):
                generate_keyfile_urandom_data(anyKeyfilePathNoQuotes)
        elif existingFileOverwrite == ("n" or "N"):
                exit(0)
        else:
                print("\n\nNot sure about that choice. Try again")
                ask_if_overwrite_existing_file(anyKeyfilePathNoQuotes)

def bookmark_keyfile_path(inputKeyfilePathNoQuotes):
        with open("PreviousKeyfilePath.txt", "w") as bookmarkKeyfilePath:
                bookmarkKeyfilePath.write(inputKeyfilePathNoQuotes)

def generate_keyfile_urandom_data(anyKeyfilePathNoQuotes):
        try:
                with open(anyKeyfilePathNoQuotes, "w") as createNewKeyfile:
